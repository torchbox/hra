...
function glossary() {
    ...
    function loadListing() {
        ...
        return fetch(apiURL + qs, { credentials: 'same-origin' }).then(function (response) {
...
function heroHeadingAlignment() {
    ...
    var $bodyWrapper = jquery$1('.standard-page-body__wrapper'),
        headingHeight = jquery$1('.hero__heading').outerHeight(),
        defaultMargin = 150,
        breakpoint = 900;

    function setMargin() {
        if (headingHeight >= defaultMargin && jquery$1(window).width() >= breakpoint) {
            $bodyWrapper.css({
